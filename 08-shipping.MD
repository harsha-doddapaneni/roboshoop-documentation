...
[Service]
User=roboshop
Environment=CART_ENDPOINT=cart.doddapaneni.shop:8080
Environment=DB_HOST=mysql.doddapaneni.shop
...
```
mysql -h mysql.doddapaneni.shop -uroot -pRoboShop@1 < /app/db/schema.sql
```
```
mysql -h mysql.doddapaneni.shop -uroot -pRoboShop@1 < /app/db/app-user.sql 
```
```
mysql -h mysql.doddapaneni.shop -uroot -pRoboShop@1 < /app/db/master-data.sql
```
